...
} from '../../redux/overhead-modal/overheadModalReducer';
...
const ConfirmationModal = () => {
  ...
  return (
    <center>
      <Modal.Body>
        <h2 className="modal-titl">
          Are you sure you want to <br /> delete these inputs
        </h2>
        <div>
          <button
            className="gena"
            ...
            onClick={async () => {
              ...
              if (
                confirmationState.type === 'delete-billboard' &&
                confirmationState.id
              ) {
                try {
                  let updatedBillboardData = [...billboardData];
                  const deletedBillboard = await billboardDataApi.delete(
                    confirmationState.id
                  );
                  const idx = billboardData.findIndex(
                    (billboard) =>
                      billboard._id === deletedBillboard.billboardData._id
                  );
                  if (idx >= 0) {
