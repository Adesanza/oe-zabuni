...
const billboardDataReducer = createSlice({
    ...
    extraReducers: (builder) => {
        builder.addCase(fetchAllBillboards.fulfilled, (state, { payload }) => payload.billboardData)
        builder.addCase(fetchAllBillboards.rejected, (state,action) => {state = []})
        builder.addCase(createABillboard.fulfilled, (state, { payload }) => {state.push(payload.billboardData)})
        builder.addCase(editABillboard.fulfilled, (state, { payload }) => {
            const idx = state.findIndex(billboard => billboard._id === payload.billboardData._id);
